...
from pulse import NSGA2  # one class i will try to edit towards Pulse
from utils import *

problem_set=([problems.numerical.cec2022_so.CEC2022TestSuit.create(x) for x in range(1, 13)])
print(f'{len(problem_set)} functions loaded in the problem_set.\n')

pso = algorithms.PSO(
    lb=jnp.full(shape=(20,), fill_value=-100),
    ub=jnp.full(shape=(20,), fill_value=100),
    pop_size=400,)
...
for x in range(n_seeds):
    ...
    for i, algo in enumerate(algorithm_list):
        ...
        for j, function in enumerate(problem_set):
            ...
            workflow = workflows.StdWorkflow(algo, function, monitors=[monitor])
...
compile_and_boxplot(functions_final_fitness, save_fig=True)
