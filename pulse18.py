# pulse18.py  ––  P17 with TIERED farthest re-pairing.
# Farthest matching alone is scale-greedy: late in a run it can pair members
# across different basins, minting chords through no-man's land (the f2
# regression). Tiered version: split the freed pool into fitness halves and
# farthest-match WITHIN each half — dissortative in geometry, assortative in
# quality. Chords then run along level sets (valley tangents) instead of
# across basins. Still zero knobs (tier = median split).
...
class PulseTiered(Algorithm):
    ...
    def step(self):
        ...
        if n_broken > 0:
            ...
            # Tiered farthest matching: quality halves, then max distance
            # within each half (level-set chords, not cross-basin ones).
            pool = list(freed)
            pool.sort(key=lambda i: float(self.fitness[i]),
                      reverse=not self.minimize)   # best first
            half = len(pool) // 2
            matched, leftovers = [], []
            for tier in (pool[:half], pool[half:]):
                tp = list(tier)
                while len(tp) >= 2:
                    i0 = tp[0]
                    rest = tp[1:]
                ...
                leftovers += tp
            if len(leftovers) == 2:
                matched.append((leftovers[0], leftovers[1]))
