# pulse15n.py  ––  pulse15 (v1) with NEUTRAL TIE semantics:
#   strict offspring win  -> success (reset fails; ride/contract continues)
#   exact tie             -> offspring enters the pair (neutral drift along the
#                            level set) but fails/mode are left untouched —
#                            navigation without immortality
#   strict parent win     -> v1 automaton (contract->extend; extend->fail->evict)
# Repairs P15T's eviction starvation (ties masked the fail counter, so the
# tie-vortex at numerical convergence never recycled) while keeping its
# plateau navigation. Zero new constants.
#
...
class PulseDirectedN(Algorithm):
    def __init__(
        self,
        pop_size: int,
        dim: int,
    ...
    def step(self):
        ...
        for k in range(self.n_pairs):
            ...
            # Offspring-priority stable ranking: children win exact ties.
            pord = [2, 3, 0, 1]
            pf = candidates_fit[pord]
            if self.minimize:
                o = torch.argsort(pf, stable=True)
            else:
                o = torch.argsort(-pf, stable=True)
            winner_a = pord[o[0].item()]
            winner_b = pord[o[1].item()]
            # Outcome classes on STRICT comparison of best offspring vs best parent
            bp = min(candidates_fit[0].item(), candidates_fit[1].item()) if self.minimize \
                else max(candidates_fit[0].item(), candidates_fit[1].item())
            bo = min(candidates_fit[2].item(), candidates_fit[3].item()) if self.minimize \
                else max(candidates_fit[2].item(), candidates_fit[3].item())
            strict_win = (bo < bp) if self.minimize else (bo > bp)
            is_tie = (bo == bp)
            offspring_best = strict_win
            ...
            if self.pair_is_new[k]:
                ...
                pass  # drift: genotype moved, bookkeeping frozen
            elif self.pair_extend[k]:
                if offspring_best:
                    self.pair_fails[k] = 0
                else:
