# pulse17d.py  ––  P17 with a PHASE-DITHERED extension ladder.
# The fixed {1x, 2x} extension grid has a dead zone: profit at sub-chord scale
# beyond the better parent (the universal endgame situation) overshoots both
# probes and forges a false "line resolved" certificate. Fix, symmetric with
# contraction's alpha-dither: keep the doubling ladder, randomize its phase.
# Probes at B + 2^(t-1) u and B + 2^t u, t ~ U(0,1): one sub-unit and one
# super-unit probe per extension generation (log-uniform octave, ratio fixed
# at 2, never degenerate). Extension gains a geometric descent gear; the
# unbounded ride is preserved. Zero new constants.
# Extend-first mode order (ride the fresh chord at full population scale,
# contract only when the exterior stops paying, evict when both are resolved)
# PLUS scale-aware re-pairing (evicted members greedily match the most distant
# freed partner, so every new lifecycle starts on the longest available chord).
# Both genes independently beat v1 11-1 at G=777; this is their composition.
    ...
    def step(self):
        ...
        for k in range(self.n_pairs):
            ...
            if self.pair_is_new[k]:
                ...
                # Dithered ladder: exponents (t-1, t) instead of (0, 1).
                m = 2.0 ** torch.rand(1, device=dev).item()
                c1 = self.population[bi] + (0.5 * m) * u
                c2 = self.population[bi] + m * u
            else:
        ...
        if n_broken > 0:
            ...
            # Greedy farthest matching (P16 gene): max initial secants.
            pool = list(freed)
            matched = []
            while len(pool) >= 2:
                i0 = pool[0]
