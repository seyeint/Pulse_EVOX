# pulse15t.py  ––  pulse15 (v1) with one change: exact-fitness TIES are
# resolved toward OFFSPRING instead of parents. A tied interpolant keeps the
# lineage drifting along the level set (free neutral diversity); a tied
# extension child keeps the ride moving across plateaus (Hooke-Jeeves plateau
# walking). Everything else is pulse15 v1 verbatim.
#
...
class PulseDirectedT(Algorithm):
    def __init__(
        self,
        pop_size: int,
        dim: int,
    ...
    def step(self):
        ...
        for k in range(self.n_pairs):
            ...
            # Rank with offspring listed first so a STABLE sort gives them
            # priority on exact ties (neutral drift toward children).
            pord = [2, 3, 0, 1]
