# pulse8.py   ––  "Pulse-CR Permutation"
#
# Cleanest version of exclusive selection: pair via random permutation.
# Each individual is used exactly once. No tournament at all during pairing —
# selection pressure comes ONLY from (μ+λ) survival selection.
#
# This is the maximally diverse pairing: every ray direction is between
# two DIFFERENT, non-repeated individuals.
...
class PulsePermutation(Algorithm):
    def __init__(
        self,
        pop_size: int,
        dim: int,
    ...
    def _pair_by_permutation(self):
        """Random permutation pairing: every individual used exactly once."""
        dev = self.population.device
        perm = torch.randperm(self.pop_size, device=dev)
        p1 = perm[::2]   # even indices
        p2 = perm[1::2]  # odd indices
        return p1, p2
    ...
    def step(self):
        ...
        p1_idx, p2_idx = self._pair_by_permutation()
