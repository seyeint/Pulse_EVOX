"""
PPO Sparsity Sweep — The "Phase Transition" Experiment
========================================================
Runs PPO at varying reward sparsity levels (K = reward every K steps).
Maps the exact degradation curve of PPO as reward becomes sparser.

EA results are INVARIANT to K (they only see episode total), so LinePulse
and OpenES scores are plotted as horizontal reference lines from existing data.

The key graph:
  X-axis: Sparsity K (log scale)
  Y-axis: Final reward
  PPO:       decays from ~970 → ~0 as K increases
  LinePulse: flat horizontal line at ~298
  OpenES:    flat horizontal line at ~54

The crossover point validates the SNR formula T² ≳ N.

Usage:
    MUJOCO_GL=osmesa python3 -u main_ppo_sweep.py 2>&1 | tee resources/ppo_baseline/sweep_run.log
"""
...
if DRY_RUN:
    ...
    N_SEEDS = 3
    ENV_NAMES = [
        "CartpoleSwingup",  # ~1.3h (260s/run × 18)
        "WalkerWalk",       # ~2.7h (540s/run × 18)
        "HumanoidWalk",     # ~4.3h (855s/run × 18)
    ]                       # Total: ~7h — fits overnight
    K_VALUES = [1, 10, 50, 200, 500, 1000]

# EA reference scores (invariant to K — they only see episode totals)
EA_SCORES = {
...
# ============================================================================
# Sparsity Wrapper — reward emitted every K steps
# ============================================================================
class SparsityRewardEnv:
    """Wraps an MjxEnv to emit accumulated reward every K steps.

    K=1:    dense (original behavior)
    K=10:   reward every 10 steps
    K=1000: fully sparse (reward only at episode end)

    Total episodic reward is IDENTICAL regardless of K.
    Only the temporal distribution changes.
    """
...
# ============================================================================
# Run PPO at a given sparsity level
# ============================================================================
def run_ppo_at_sparsity(env_name: str, k: int, seed: int, num_timesteps: int):
    ...
    def progress_fn(num_steps, metrics):
        ...
        # Only print first/last eval to avoid spam during sweep
...
def main():
    ...
    print(f"Mode: {'DRY RUN' if DRY_RUN else 'PAPER QUALITY'} — SPARSITY SWEEP")
    ...
    for env_name in ENV_NAMES:
        ...
        print(f"  {'K':>6}  {'PPO median':>12}  {'PPO peak':>10}  {'LinePulse':>10}  {'OpenES':>8}  {'Winner'}")
        print(f"  {'─'*65}")

        for k in K_VALUES:
            results = all_results[k]
            med = np.median([r["final_reward"] for r in results])
            peak = np.median([r["peak_reward"] for r in results])
            lp = ea.get("LinePulse", 0)
            oes = ea.get("OpenES", 0)
            winner = "PPO" if med >= lp else "LinePulse"
            print(f"  {k:>6}  {med:>12.1f}  {peak:>10.1f}  {lp:>10.1f}  {oes:>8.1f}  ← {winner}")

        # Incremental save after each env
        _save_partial(all_env_results, ENV_NAMES[:ENV_NAMES.index(env_name)+1])
    ...
    # ========================================================================
    print(f"\n\n{'='*70}")
    print(f"SPARSITY SWEEP — ALL ENVIRONMENTS  (total: {total_time/60:.1f} min)")
    print(f"{'='*70}")
    ...
    # Final save
    _save_partial(all_env_results, ENV_NAMES)


def _save_partial(all_env_results, env_names_done):
    os.makedirs("resources/ppo_baseline", exist_ok=True)
    out_path = "resources/ppo_baseline/sweep_results.json"
    save_data = {}
